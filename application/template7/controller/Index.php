<?php
namespace app\template7\controller;
class Index extends Base
{
    ...
    public function index()
    {
        ...
        if ($this->request->isGet())
        {
            ...
            // 读取MBA热点关注
            $this->data['hotData'] = db('programa_article')->where('status',0)->where('type',3)->order('ctime desc')->limit(5)->field('content',true)->select();
            // 读取MBA活动预告广告
            ...
            // 读取考试资讯
            $this->data['examData'] = db('programa_article')->where('status',0)->where('type',8)->order('ctime desc')->limit(7)->field('content',true)->select();
            // 读取院校发布上方广告
